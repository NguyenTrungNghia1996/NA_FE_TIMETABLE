<template>
  <div class="p-2 md:p-4 bg-white min-h-full">
    ...
    <a-modal
      ...
      :title="isEdit ? 'Chỉnh sửa cấp học' : 'Thêm mới cấp học'"
      @cancel="handleCancel"
      :width="600">
      <a-form ref="formRef" :model="formState" layout="vertical" :rules="rules">
        ...
        <a-form-item
          ...
          :wrapper-col="{ span: 24 }">
          <a-textarea
            v-model:value="formState.ghichu"
            :rows="4"
...
const fetchData = async param => {
  try {
    loading.value = true;
    const { data, error } = await RestApi.school_level.list({ params: param });
...
const handleOk = async () => {
  try {
    ...
    message.error(
      error.message || error.response?.data?.message || "Đã xảy ra lỗi khi lưu thông tin cấp học",
    );
  } finally {
    await fetchData({ ...param.value });
