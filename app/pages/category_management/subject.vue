<template>
  <div class="p-2 md:p-4 bg-white min-h-full">
    <div class="flex flex-col lg:flex-row gap-2 mb-4 lg:items-center">
      ...
      <div class="flex flex-col w-full gap-2 lg:w-auto lg:flex-row lg:flex-wrap lg:justify-end">
        <a-button @click="resetForm" class="w-full lg:w-auto">
          <span class="md:inline">Đặt lại</span>
        </a-button>
        <a-button
          @click="openBusyManager"
          class="w-full lg:w-auto"
          :disabled="!settingStore.currentPermission">
          Cài đặt tiết tránh xếp
        </a-button>
        <a-button
          @click="openDrawer"
          class="w-full lg:w-auto"
          :disabled="!settingStore.currentPermission">
          Cài đặt tiết cố định
        </a-button>
        <a-button
          @click="SubjectCombinationDrawer = true"
          class="w-full lg:w-auto"
          :disabled="!settingStore.currentPermission">
          Tổ hợp môn
        </a-button>
        <a-button
          ...
          class="w-full lg:w-auto"
          :disabled="!settingStore.currentPermission">
          <span class="lg:inline">Thêm mới</span>
        </a-button>
      </div>
    </div>
    ...
    <a-modal
      ...
      <a-form ref="formRef" :model="formState" layout="vertical" :rules="rules">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <SelectClassroomType
            v-model="formState.Id_loai_phong_hoc"
            name="Id_loai_phong_hoc"
            :rules="rules.Id_loai_phong_hoc" />
          <SelectKnowledge
            v-model="formState.Id_khoi_kien_thuc"
            name="Id_khoi_kien_thuc"
            :multiple="true" />
          <a-form-item label="Số tiết tối đa một ca" name="So_tiet_toi_da_mot_ca">
            <a-input-number
              v-model:value="formState.So_tiet_toi_da_mot_ca"
              :precision="0"
              style="width: 100%" />
          </a-form-item>
          <a-form-item label="Số tiết tối đa hai ca" name="So_tiet_toi_da_hai_ca">
            <a-input-number
              v-model:value="formState.So_tiet_toi_da_hai_ca"
              :precision="0"
              style="width: 100%" />
          </a-form-item>
          <SelectClassroom
    ...
    <a-modal
      v-model:open="busy_modal"
      title="Cài đặt tiết tránh xếp"
      @cancel="handleBusyCancel"
      :width="600"
      :footer="null">
    ...
    <a-drawer
      height="100vh"
      title="Cài đặt tiết cố định"
      placement="bottom"
      v-model:open="statusDrawer"
      @close="onCloseDrawer">
      <FixedLesson ref="fixedLessonRef" />
    </a-drawer>
    <a-drawer
      v-model:open="SubjectCombinationDrawer"
      title="Tổ Hợp môn"
      @close="onCloseSubjectCombinationDrawer"
      :footer="null"
      height="100vh"
      placement="bottom">
...
const columns = [
  ...
  { title: "Tên môn học", dataIndex: "ten", key: "ten", ellipsis: true, width: 120 },
  {
    ...
    dataIndex: "ten_loai_phong_hoc",
    key: "ten_loai_phong_hoc",
    ellipsis: true,
    width: 150,
  },
  ...
  {
    title: "Số tiết/ca",
    dataIndex: "so_tiet_toi_da_mot_ca",
    key: "so_tiet_toi_da_mot_ca",
    align: "center",
    width: 100,
  },
  {
    title: "Số tiết/2 ca",
    dataIndex: "so_tiet_toi_da_hai_ca",
    key: "so_tiet_toi_da_hai_ca",
    align: "center",
    width: 120,
  },
...
const busyRowClassName = record =>
  selectedSubject.value && record.id === selectedSubject.value.id ? "active-row" : "";
  ...
  try {
    ...
    if (data.value?.status === "success") {
      Object.assign(formState, {
        ...
        Id_loai_phong_hoc:
          data.value.data.id_loai_phong_hoc != 0 ? data.value.data.id_loai_phong_hoc : undefined,
