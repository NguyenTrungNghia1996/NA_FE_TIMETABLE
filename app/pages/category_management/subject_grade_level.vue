  ...
  <div class="p-4 bg-white min-h-screen space-y-2">
    ...
    <div class="flex justify-end">
      <div class="flex space-x-3">
        <a-button type="primary" @click="handleAvoid"> Tiết tránh xếp </a-button>
            ...
            <template v-else-if="isShiftColumn(column.key)">
              <template v-if="getShiftField(column.key) === 'period'">
                <a-input-number
                  ...
                  class="w-full"
                  @change="
                    value => onShiftNumberChange(record, getShiftId(column.key), 'period', value)
                  " />
              ...
              <template v-else>
                <a-input-number
                  ...
                  class="w-full"
                  @change="
                    value => onShiftNumberChange(record, getShiftId(column.key), 'group', value)
                  " />
            ...
            <template v-if="column.key === 'action'">
              <div class="flex justify-center space-x-2">
                <a-switch
                  ...
                  size="small"
                  @change="onEditableChange(record)" />
              </div>
            </template>
          </template>
        </a-table>
...
const displayColumns = computed(() => {
  const selectedShiftId =
    filters.shift !== undefined && filters.shift !== null ? String(filters.shift) : undefined;
...
watch(
  ...
  val => {
    val.forEach(r => {
      const total = Object.values(r.shifts || {}).reduce(
        (sum, shift) => sum + (Number(shift.period) || 0),
        0,
      );
      r.weekly = total;
    });
...
const convertFromApi = data => {
  ...
  records.forEach(record => {
    ...
    record.weekly = Object.values(record.shifts).reduce(
      (sum, shift) => sum + (Number(shift.period) || 0),
      0,
    );
  });
...
const convertToApi = () => {
  return {
    ...
    ds_Mon: subjects.value.map(mon => ({
      id_Mon: mon.id,
      ten_mon: mon.name,
      ds_Ca: (mon.shiftOrder?.length
        ? mon.shiftOrder
        : availableShifts.value.map(shift => shift.id)
      )
