<template>
  <div class="p-2 md:p-4 bg-white min-h-full">
    <div class="flex flex-col lg:flex-row gap-2 mb-4 lg:items-center">
      ...
      <div class="flex flex-col w-full gap-2 lg:w-auto lg:flex-row lg:flex-wrap lg:justify-end">
        <a-button @click="resetForm" class="w-full lg:w-auto">
          <span class="lg:inline">Đặt lại</span>
        </a-button>
        <a-button
          type="primary"
          @click="drawerAssignmentOpen = true"
          class="w-full lg:w-auto"
          :disabled="!settingStore.currentPermission">
          <span class="lg:inline">Chuyên môn của Giáo viên</span>
        </a-button>
        <a-button
          type="primary"
          @click="drawerAvoidOpen = true"
          class="w-full lg:w-auto"
          :disabled="!settingStore.currentPermission">
          <span class="lg:inline">Tiết tránh xếp</span>
        </a-button>
        <a-button
          type="primary"
          @click="showModal"
    ...
    <ClientOnly class="overflow-x-auto">
      <a-table
        ...
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-if="column.key === 'ho_va_ho_dem'">
    ...
    <a-modal
      ...
      :title="isEdit ? 'Chỉnh sửa giáo viên' : 'Thêm mới giáo viên'"
      @cancel="handleCancel"
      :width="600">
      <a-form ref="formRef" :model="formState" layout="vertical" :rules="rules">
        <a-form-item
          ...
          :wrapper-col="{ span: 24 }">
          <a-input
            v-model:value="formState.ho_va_ho_dem"
        ...
        <SelectExpertise
          v-model="formState.id_to_chuyen_mon"
          name="id_to_chuyen_mon"
          :multiple="true"
          :rules="rules.id_to_chuyen_mon" />
        <SelectSchoolSite
          v-model="formState.id_diem_truong"
          name="id_diem_truong"
          :multiple="true"
          :rules="rules.id_diem_truong" />
      </a-form>
    ...
    <a-drawer
      ...
      title="Thiết lập tiết tránh xếp của giáo viên"
      :footer="null"
      height="100vh"
      placement="bottom"
      @close="closeTeacherAvoid">
      <TeacherAvoid ref="avoidRef" />
    </a-drawer>
    <a-drawer
      v-model:open="drawerAssignmentOpen"
      title="Chuyên môn của Giáo viên"
      :footer="null"
      height="100vh"
      placement="bottom"
...
const columns = [
  ...
  {
    title: "Tổ chuyên môn",
    dataIndex: "ten_to_chuyen_mon",
    key: "ten_to_chuyen_mon",
    ellipsis: true,
  },
