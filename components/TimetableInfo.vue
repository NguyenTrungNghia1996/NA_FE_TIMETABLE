    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="bg-white p-4 rounded-md shadow-lg col-span-2">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
          <a-button v-for="btn in ['Xếp Phòng chức năng', 'Xếp GVCN', 'Xếp Môn học', 'Xếp Giáo viên', 'Xếp Nhóm', 'Xếp Lớp', 'Xếp Phòng học']" :key="btn" type="primary" class="bg-blue-500 border-blue-500" @click="arrangePartial(btn)">
            {{ btn }}
          </a-button>
        </div>
      </div>
