    ...
    <div class="w-full space-y-1.5">
      ...
      <a-spin :spinning="loading">
        <section class="grid items-start gap-1.5 pt-3 xl:grid-cols-[minmax(0,1fr)_340px]">
          <aside class="grid grid-cols-1 gap-1.5 xl:hidden">
            <a-card :bordered="false" class="side-panel-card" :body-style="{ padding: '0' }">
              ...
              <a-table
                size="small"
                :columns="sideColumns"
                :data-source="filteredWaitingList"
                :pagination="false"
                :scroll="{ y: sideTableScrollY }"
                :row-key="record => `mobile-waiting-${record.id || record.order}`"
              />
            </a-card>
            ...
            <a-card :bordered="false" class="side-panel-card" :body-style="{ padding: '0' }">
              ...
              <a-table
                ...
                :data-source="filteredUnassignedList"
                :pagination="false"
                :scroll="{ y: sideTableScrollY }"
                :row-key="record => `mobile-unassigned-${record.id || record.order}`"
              >
                <template #bodyCell="{ column, record }">
                  <template v-if="column.key === 'action'">
                    <a-button
                      size="small"
                      class="!bg-[#4b9cc9] !border-[#4b9cc9] !text-white hover:!bg-[#3f86ad] hover:!border-[#3f86ad]"
                      :loading="drawingTeacherId === record.id"
                      :disabled="!showDrawAction || disableDrawActions"
                      @click="emit('draw-unassigned', record)"
                    >
                      Bốc
                    </a-button>
                  </template>
          ...
          <div class="min-w-0">
            <div
              v-if="!filteredBuildings.length && !loading"
            ...
            <a-collapse
              v-else
              v-model:activeKey="activeBuildingKeys"
              :bordered="false"
            ...
            >
              <a-collapse-panel v-for="building in filteredBuildings" :key="building.id">
                ...
                <a-collapse
                  v-model:activeKey="activeFloorKeys[building.id]"
                  :bordered="false"
                  class="floor-collapse"
                >
                      ...
                      <div class="space-y-1.5">
                        ...
                        >
                          <div class="flex items-center justify-between border-b border-slate-300 bg-[#f1f7f9] px-2.5 py-1.5 text-sm text-slate-700">
                            ...
                            <a-popconfirm
                              v-if="showResetActions && group.supervisorId"
                              title="Hủy kết quả giám sát này?"
                              ok-text="Đồng ý"
                              cancel-text="Hủy"
                            ...
                            >
                              <a-button
                                ...
                                danger
                                size="small"
                                :loading="resettingSupervisorId === group.supervisorId"
                                :disabled="disableResetActions"
                              >
                                <template #icon>
                          ...
                          <div class="grid gap-1.5 p-1.5 md:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-5">
                            ...
                            >
                              <div class="mb-1.5 flex items-start justify-between gap-1.5">
                                ...
                                <a-popconfirm
                                  v-if="showResetActions && room.roomId"
                                  title="Hủy kết quả phòng này?"
                                  ok-text="Đồng ý"
                                  cancel-text="Hủy"
                                ...
                                >
                                  <a-button
                                    ...
                                    danger
                                    size="small"
                                    :loading="resettingRoomId === room.roomId"
                                    :disabled="disableResetActions"
                                  >
                                    <template #icon>
                                      <Icon name="ant-design:delete-outlined" size="15" />
          ...
          <aside class="hidden grid-cols-1 gap-1.5 space-y-1.5 xl:grid xl:sticky xl:top-2">
            <a-card :bordered="false" class="side-panel-card side-panel-fixed" :body-style="{ padding: '0' }">
              ...
              <a-table
                ...
                :pagination="false"
                :scroll="{ y: sideTableScrollY }"
                :row-key="record => `waiting-${record.id || record.order}`"
              />
            </a-card>
            ...
            <a-card :bordered="false" class="side-panel-card side-panel-fixed" :body-style="{ padding: '0' }">
              ...
              <a-table
                ...
                :pagination="false"
                :scroll="{ y: sideTableScrollY }"
                :row-key="record => `unassigned-${record.id || record.order}`"
              >
                <template #bodyCell="{ column, record }">
                  <template v-if="column.key === 'action'">
                    <a-button
      ...
      floors: (building.floors || [])
        ...
        .map(floor => ({
          ...
          supervisorGroups: (floor.supervisorGroups || [])
            ...
            .map(group => ({
              ...group,
              rooms: group.groupMatches ? (group.rooms || []) : (group.rooms || []).filter(room => {
