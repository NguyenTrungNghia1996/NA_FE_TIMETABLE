...
const fetchTinh = async (searchValue = "") => {
  loading.value = true;
  try {
    const searchTerm = (searchValue || "").trim();
    const { data, error } = await RestApi.tinh.list({ params: { search: searchTerm } });
    ...
    options.value = [];
    message.error(error?.message || error?.value?.data?.message || "Không thể tải danh sách tỉnh");
  } finally {
    loading.value = false;
  }
};
