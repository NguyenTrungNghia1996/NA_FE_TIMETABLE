...
const props = defineProps({
  ...
  label: { type: String, default: "Tổ hợp môn ôn" },
  name: { type: String, default: "tohopmonon" },
  multiple: { type: Boolean, default: false },
  placeholder: { type: String, default: "Chọn tổ hợp môn ôn" },
...
const fetchReviewSubjectCombinations = async (keyword = "") => {
  loading.value = true;
  try {
    ...
    const { data, error } = await RestApi.review_subject_combination.list({ params });
    if (data.value?.status === "success") {
      ...
      if (props.autoSelectFirst && !searchTerm && isEmptyValue(props.modelValue) && options.value.length > 0) {
        const first = options.value[0];
        emit("update:modelValue", props.multiple ? [first.value] : first.value);
      }
    } else {
      throw new Error(error.value?.data?.message || data.value?.message || "Không thể tải danh sách tổ hợp môn ôn");
    }
  } catch (error) {
    options.value = [];
    message.error(error?.message || "Không thể tải danh sách tổ hợp môn ôn");
  } finally {
    loading.value = false;
  }
};
...
const debouncedFetch = debounce(val => {
  fetchReviewSubjectCombinations((val || "").trim());
}, 300);
...
const onClear = () => {
  ...
  search.value = "";
  fetchReviewSubjectCombinations("");
};
...
await fetchReviewSubjectCombinations();
</script>
