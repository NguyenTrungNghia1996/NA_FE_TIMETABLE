...
const fetchCaHoc = async (search = "") => {
  loading.value = true;
  try {
    const { data, error } = await RestApi.school_shift.list({ params: { search } });
    ...
    options.value = [];
    message.error(error?.message || error?.value?.data?.message || "Không thể tải danh sách ca học");
  } finally {
    loading.value = false;
  }
};
