...
const props = defineProps({
  ...
  label: { type: String, default: "Loại bài kiểm tra" },
  name: { type: String, default: "loaikiemtra" },
  multiple: { type: Boolean, default: false },
  placeholder: { type: String, default: "Chọn loại bài kiểm tra" },
...
const fetchReviewTestTypes = async (search = "") => {
  loading.value = true;
  try {
    const searchTerm = (search || "").trim();
    const params = { pageIndex: 1, pageSize: 1000 };
    if (searchTerm) params.search = searchTerm;
    const { data, error } = await RestApi.review_test_type.list({ params });
    ...
    if (data.value?.status === "success" && Array.isArray(items)) {
      ...
      throw new Error(error?.value?.data?.message || "Không thể tải danh sách loại bài kiểm tra");
    }
  } catch (err) {
    options.value = [];
    message.error(err?.message || err?.value?.data?.message || "Không thể tải danh sách loại bài kiểm tra");
  } finally {
    loading.value = false;
  }
};
