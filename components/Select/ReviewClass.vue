...
const props = defineProps({
  ...
  label: { type: String, default: "Lớp ôn tập:" },
  name: { type: String, default: "lopontap" },
  multiple: { type: Boolean, default: false },
  placeholder: { type: String, default: "Chọn lớp ôn tập" },
...
const fetchReviewClasses = async (search = "") => {
  ...
  try {
    ...
    if (props.id_khoi) params.id_khoi = props.id_khoi;
    ...
    options.value = [];
    message.error(error?.message || error?.value?.data?.message || "Không thể tải danh sách lớp ôn tập");
  } finally {
    loading.value = false;
  }
};
