...
const props = defineProps({
  ...
  name: { type: String, default: "id_dan_toc" },
...
const fetchDanToc = async (searchValue = "") => {
  loading.value = true;
  try {
    const searchTerm = (searchValue || "").trim();
    const { data, error } = await RestApi.dan_toc.list({ params: { search: searchTerm } });
    ...
    options.value = [];
    message.error(error?.message || error?.value?.data?.message || "Không thể tải danh sách dân tộc");
  } finally {
    loading.value = false;
  }
};
