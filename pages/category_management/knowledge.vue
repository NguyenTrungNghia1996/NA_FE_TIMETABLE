<template>
  <div class="p-2 md:p-4 bg-white min-h-full">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 mb-4">
      <a-input-search v-model:value="searchText" placeholder="Tìm kiếm khối kiến thức..." enter-button @search="handleSearch" class="w-full md:w-1/3" />
...
const rules = reactive({
  ten: [
    { required: true, message: "Vui lòng nhập tên khối kiến thức", trigger: "blur" },
...
const handleOk = async () => {
  try {
    ...
    if (isEdit.value) {
      const { data, error } = await RestApi.knowledge.update({ body: { ...formState } });
      if (data.value?.status === "success") {
        message.success(data.value?.message || "Cập nhật khối kiến thức thành công");
      } else {
      ...
      const { data, error } = await RestApi.knowledge.create({ body: { ...formState } });
      if (data.value?.status === "success") {
        message.success(data.value?.message || "Tạo mới khối kiến thức thành công");
      } else {
