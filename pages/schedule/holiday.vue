<template>
  <div class="p-2 md:p-4 bg-white min-h-full">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 mb-4">
      <a-input-search v-model:value="searchText" placeholder="Tìm kiếm ngày nghỉ..." enter-button @search="handleSearch" class="w-full md:w-1/3" />
    ...
    <a-modal v-model:open="visible" :title="isEdit ? 'Chỉnh sửa ngày nghỉ' : 'Thêm mới ngày nghỉ'" @cancel="handleCancel" :width="640">
      <a-form ref="formRef" :model="formState" layout="vertical" :rules="rules">
        <a-form-item label="Tên ngày nghỉ" name="ten" :label-col="{ span: 24 }" :wrapper-col="{ span: 24 }">
          <a-input v-model:value="formState.ten" placeholder="Nhập tên ngày nghỉ" :maxlength="100" show-count />
        </a-form-item>
...
const columns = [
  ...
  { title: 'Tên ngày nghỉ', dataIndex: 'ten', key: 'ten', ellipsis: true },
...
const rules = reactive({
  ten: [
    { required: true, message: 'Vui lòng nhập tên ngày nghỉ', trigger: 'blur' },
...
const fetchData = async (p) => {
  try {
    loading.value = true;
    const { data, error } = await RestApi.holiday.list({ params: p });
...
const buildPayload = () => {
  ...
  return {
    ...
    ten: formState.ten,
    // Use local date format to avoid timezone shifting one day backward
...
const deleteItem = async (id) => {
  try {
    const { data, error } = await RestApi.holiday.delete({ params: { Id: id } });
    if (data.value?.status === 'success') {
