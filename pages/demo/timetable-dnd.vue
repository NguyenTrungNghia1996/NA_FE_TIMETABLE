  ...
  <div class="p-4 bg-white space-y-8" @click="closeMenu">
    ...
    <div v-if="currentClass" class="space-y-4">
      ...
      <div class="flex flex-wrap gap-2 text-sm">
        <span v-for="stat in subjectStats" :key="stat.subject"> {{ stat.subject }}: {{ stat.count }}/{{ stat.max }} </span>
      </div>
        ...
        <table class="min-w-full table-fixed border border-gray-200">
          ...
          <tbody>
            <tr v-for="(tiet, tIndex) in ca.ds_Ngay[0].ds_Tiet" :key="tiet.id">
              ...
              <td v-for="(day, dIndex) in ca.ds_Ngay" :key="day.id" class="border p-2 text-center relative w-32 h-20 overflow-hidden" :class="cellClass(currentClassIndex, caIndex, dIndex, tIndex)" @dragstart="dragStart($event, currentClassIndex, caIndex, dIndex, tIndex)" @dragenter.prevent="dragEnter($event, currentClassIndex, caIndex, dIndex, tIndex)" @dragover.prevent="dragOver($event, currentClassIndex, caIndex, dIndex, tIndex)" @drop.prevent="drop($event, currentClassIndex, caIndex, dIndex, tIndex)" @dragend="dragEnd" @contextmenu.prevent="openMenu($event, currentClassIndex, caIndex, dIndex, tIndex)" @click="day.ds_Tiet[tIndex].teacherId && selectTeacherLesson(day.ds_Tiet[tIndex].teacherId)" :draggable="!day.ds_Tiet[tIndex].isBreak">
          ...
          <table class="min-w-full table-fixed border border-gray-200 text-sm">
            ...
            <tbody>
              <tr v-for="(tiet, ti) in ca.ds_Ngay[0].ds_Tiet" :key="ti">
                ...
                <td v-for="(day, di) in ca.ds_Ngay" :key="di" class="border p-1 text-center w-32 h-20 overflow-hidden" :class="teacherCellClass(currentTeacher.id, ci, di, ti, day.ds_Tiet[ti].className)" @click="teacherCellClick(ci, di, ti, day.ds_Tiet[ti].className, currentTeacher.id)">
                  <span v-if="day.ds_Tiet[ti].className" class="line-clamp-2 block"> {{ day.ds_Tiet[ti].className }} - {{ day.ds_Tiet[ti].subject }} </span>
                  <span v-else>-</span>
                </td>
              </tr>
            </tbody>
    ...
    <a-modal v-model:open="subjectSelect.visible" title="Chọn môn học" @ok="confirmSubject" @cancel="cancelSubject" :footer="null">
      <a-select v-model:value="subjectSelect.value" class="w-full">
        <a-select-option v-for="o in subjectSelect.options" :key="o.id + '-' + o.subject" :value="o.id + '-' + o.subject"> {{ o.subject }} - {{ o.name }} </a-select-option>
      </a-select>
