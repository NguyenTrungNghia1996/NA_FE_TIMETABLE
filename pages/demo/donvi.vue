<template>
  <div class="max-w-3xl mx-auto p-6 space-y-6">
    <h1 class="text-2xl font-bold">Demo: SelectUnit (Đơn vị)</h1>

    <ClientOnly>
      <a-form :model="form" @finish="submit" layout="vertical">
        <SelectUnit
          v-model="form.unitMultiple"
          label="Chọn nhiều đơn vị"
          name="unitMultiple"
          :multiple="true"
        />

        <SelectUnit
          v-model="form.unitSingle"
          label="Chọn một đơn vị"
          name="unitSingle"
        />

        <SelectUnit
          v-model="form.unitDisabled"
          label="Đơn vị bị khóa"
          name="unitDisabled"
          :disabled="true"
        />

        <a-button type="dashed" @click="modalOpen = true">Chọn đơn vị trong Modal</a-button>

        <a-button html-type="submit" type="primary" class="mt-4">Gửi</a-button>
      </a-form>

      <a-modal v-model:open="modalOpen" title="Chọn đơn vị" @ok="submitModal">
        <SelectUnit
          v-model="modalForm.modalUnit"
          label="Đơn vị modal"
          name="modalUnit"
        />
      </a-modal>
...
const propsTable = [
  ...
  { prop: 'label', type: 'String', default: 'Đơn vị', desc: 'Nhãn hiển thị trên form item' },
  { prop: 'name', type: 'String', default: 'donvi', desc: 'Tên trường trong form' },
  { prop: 'multiple', type: 'Boolean', default: 'false', desc: 'Cho phép chọn nhiều giá trị hay không' },
  { prop: 'placeholder', type: 'String', default: 'Chọn đơn vị', desc: 'Placeholder hiển thị' },
