<template>
  <div class="p-2 md:p-4 bg-white min-h-full">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 mb-4">
      <a-input-search v-model:value="searchText" placeholder="Tìm kiếm loại bài kiểm tra..." enter-button @search="handleSearch" class="w-full md:w-1/3" />
    ...
    <ClientOnly class="overflow-x-auto">
      <a-table :columns="columns" :data-source="dataSource" :pagination="pagination" :loading="loading" :scroll="{ x: '800' }" row-key="id" @change="handleTableChange" bordered size="small">
    ...
    <a-modal v-model:open="visible" :title="isEdit ? 'Chỉnh sửa loại bài kiểm tra' : 'Thêm mới loại bài kiểm tra'" @cancel="handleCancel" :width="600">
      <a-form ref="formRef" :model="formState" layout="vertical" :rules="rules">
        <a-form-item label="Tên loại bài kiểm tra" name="ten" :label-col="{ span: 24 }" :wrapper-col="{ span: 24 }">
          <a-input v-model:value="formState.ten" placeholder="Nhập tên loại bài kiểm tra" :maxlength="50" show-count />
        </a-form-item>
...
const columns = [
  ...
  { title: "Tên loại bài kiểm tra", dataIndex: "ten", key: "ten", ellipsis: true },
...
const rules = reactive({
  ten: [
    { required: true, message: "Vui lòng nhập tên loại bài kiểm tra", trigger: "blur" },
    { max: 50, message: "Tên loại bài kiểm tra tối đa 50 ký tự", trigger: "blur" },
  ],
});
...
const fetchData = async p => {
  try {
    loading.value = true;
    const { data, error } = await RestApi.review_test_type.list({ params: p });
...
const handleOk = async () => {
  try {
    ...
    if (isEdit.value) {
      const { data, error } = await RestApi.review_test_type.update({ body: payload });
      if (data.value?.status === "success") {
      ...
      delete payload.id;
      const { data, error } = await RestApi.review_test_type.create({ body: payload });
      if (data.value?.status === "success") {
...
const deleteItem = async id => {
  try {
    const { data, error } = await RestApi.review_test_type.delete({ params: { id } });
    if (data.value?.status === "success") {
